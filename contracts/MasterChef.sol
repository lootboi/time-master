...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // dETH reflects ether to holders, if deposited here, it will be lost.
    // This function allows the team to recover any dividends that are sent to the contract.
    // These funds are then transferred to the dev address, where they are used to buy back dETH and burn it.
    function retrieveDividends() external onlyOwner {
        require(msg.sender == devAddress, "You are not the dev");
        msg.sender.transfer(address(this).balance);
    }

    // Allows the contract to recieve ether from dETH dividends
